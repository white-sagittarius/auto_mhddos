...
# Оновлення: станом на 5.04 даний метод атаки блокується Google'ом. Тож поки що переходьте на інші клауд провайдери - Digital Ocean, Vultr, AWS, Azure тощо - та встановлюйте скрипти з інших репозиторіїв. Деякі провайдерів дають безоплатні кредити в різній кількості, їх вистачить на кілька тижнів роботи. Треба тільки витратити деякий час та почитати інструкції з різних груп атак - їх чимало.
...
- вже запущено 5 вкладок. На кожний аккаунт можна запускати максимум 5 вкладок. Щоб подивитися чи зупинити, що зараз все запущено, можна використовувати пункт меню Runtime -> Manage sessions
- акаунт заблоковано. Все інше на ньому може працювати (пошта, сам гугл диск), але Colaboratory недоступний. У такому разі єдиний вихід - створити новий гугл аккаунт, записати на нього *нові* файли і почати наново
...
13. Можна додати автоматичне очищення логів програми кожні 5 хвилин. Для цього треба прописати в консолі браузера (більш детально розписано у інструкції з картинками): `function ClearOutput(){ document.querySelector("iron-icon[command = 'clear-focused-or-selected-outputs']").click() } setInterval(ClearOutput,300000)`
