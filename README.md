...
mac, linux и windows (power shell): 
`docker run --pull=always --rm -v ${PWD}:/output -e "count=5" jjerboa/sflower:latest`

windows (cmd): 
`docker run --pull=always --rm -v %cd%:/output -e "count=5" jjerboa/sflower:latest`

(більш детально про те, як запускати  команди - у інструкції зі скриншотами, за посиланням вище)
...
Увага! Якщо ваш акаунт таки був заблокований (тобто, сам аккаунт може працювати нормально, а от Google Colaboratory не запускатися) - треба створити новий аккаунт і спробувати з нього. Нові файли на старому вже заблоченому аккаунті не будуть працювати
